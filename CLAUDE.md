...
│       ├── Focalizacion.jsx (/admin/focalizacion) — manage focalización and visitas sin focalizar without drilling into Convenios: a proyecto filter, then an accordion of convenios (only the ones with visita_focalizada/visita_sin_focalizar metas), each expanding to stack the full `PanelFocalizacionMeta`/`PanelAsignacionesMeta` of each of its qualifying metas (metas "Manual" don't show up here, those stay in the Convenios accordion). One shared fetch (useEntidad per entity) for the whole tree, no matter how many convenios/metas render
