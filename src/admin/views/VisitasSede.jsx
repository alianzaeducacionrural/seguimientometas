...
export default function VisitasSede() {
  ...
  const filtradas = filas.filter((f) => {
    if (proyectoId && !String(f.convenio?.proyectos_ids).split(',').includes(proyectoId)) return false
  ...
  return (
    <section>
      ...
      <p>
        Realizadas: <strong>{realizadas}</strong> · Programadas: <strong>{programadas}</strong>
        {' '}· Proyectado (realizadas + programadas): <strong>{realizadas + programadas}</strong>
        {' '}de {filtradas.length} focalizadas
      </p>

      {filtradas.length === 0 ? (
        <p>No hay focalización que coincida con los filtros.</p>
      ) : (
